          ...
          <div className="detail-grid">
            ...
            <div className="detail-item">
              ...
              <ul className="bullet-list">
                ...
                <p>
                  改善前：各クラスが直接他のクラスを参照し、依存関係が複雑化していた。<br />
                  改善後：ID参照やイベントシステム、コンポーネント指向を導入し、クラス間の依存を最小限に抑制。
                </p>
                <li>C++20のモジュール機能を活用し、ビルド時間の短縮とコードの可読性向上を実現。</li>
                <p>
                  改善前：C++14を使用しており、ヘッダーファイルの依存関係がビルド時間を増加させていた。<br />
                  改善後：C++20のモジュール機能を導入し、ヘッダーファイル依存を削減。ビルド時間が大幅に短縮され、コードの可読性と保守性が向上。
                </p>
                <li>テンプレートや関数ポインタを習得。</li>
                <p>
                  改善前：関数ポインタやテンプレートの使用経験が少なく、コードの柔軟性が低かった。<br />
                  改善後：関数ポインタやテンプレートを用いた汎用的なコンポーネント設計を実装。コードの柔軟性と再利用性が向上。
                </p>
                <li>コンポーネント指向を採用し、機能の組み合わせの柔軟性とコードの再利用性を向上。</li>
                <p>
                  改善前：オブジェクト指向設計に基づき、機能がクラスに固定されていた。<br />
                  改善後：コンポーネント指向を導入し、オブジェクトに機能を動的に追加・組み合わせ可能に。コードの再利用性と拡張性が向上。
                </p>
                <li>メモリマップドファイルを利用して大容量データの読み込みパフォーマンスを最適化。</li>
                <p>
                  改善前：従来のファイル読み込み方式では、大容量データの読み込みに時間がかかり、パフォーマンスが低下していた。<br />
                  改善後：メモリマップドファイルを導入し、ファイルI/Oのオーバーヘッドを削減。大容量データの読み込み速度が大幅に向上し、ゲームの起動時間と動作のスムーズさが改善。
                </p>
                <li>非同期処理の実装</li>
                <p>
                  改善前：同期的な処理が多く、リソースの読み込みやシーン遷移時にゲームが一時停止していた。<br />
                  改善後：非同期処理を導入。リソースの読み込みやシーン遷移をスムーズに行えるようになり<br />
                  ゲームのパフォーマンスとユーザー体験が向上。
                </p>
                <li>シーン管理の柔軟性と拡張性を向上。</li>
                <p>
                  改善前：シーン遷移の管理が固定的で、新しいシーン操作の追加が困難であった。<br />
                  改善後：コマンドパターンを採用し、シーン操作の柔軟性と拡張性を向上。<br />
                  新しいシーン操作の追加が容易になり、コードの保守性が高まった。
                </p>
                <li>マップデータのメモリ使用量とアクセス効率を最適化。</li>
                <p>
                  改善前：マップデータを二次元配列で管理しており、大規模なマップではメモリ使用量が増加し、アクセス効率が低下していた。<br />
                  改善後：チャンク化された一次元配列でマップデータを管理。メモリ使用量とアクセス効率が最適化され、大規模なマップデータの分割管理が可能となり、メモリの断片化を防止。
                </p>
                <li>独自の2Dコライダーシステムを実装し、衝突判定の効率化とライフサイクル管理を改善。</li>
                <p>
                  改善前：衝突判定システムが限定的で、コライダーの管理と衝突処理の拡張が困難であった。<br />
                  改善後：独自の2Dコライダーシステムを構築。IDでの管理を採用し、コライダーのライフサイクル管理を効率化。<br />
                  ビジターパターンを用いた柔軟な衝突処理の実装が可能となり、システムの拡張性が向上。
                </p>
                <li>UI管理システムを構築</li>
                <p>
                  改善前：UI要素の管理が分散しており、ライフサイクル管理とイベント処理が煩雑であった。<br />
                  改善後：UI管理システムを導入し、UI要素の作成・配置・イベント処理を効率化。<br />
