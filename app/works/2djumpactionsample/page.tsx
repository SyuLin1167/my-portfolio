...
export default function JumpActionPage() {
  ...
  const downloads = {
    executable: {
      ...
      href: `${assetBase}/assets/2DJumpActionSample/2DJumpActionSample-exe.zip?download=` as string | null,
    },
    sourceZip: {
      label: "ソースコード",
      href: `${assetBase}/assets/2DJumpActionSample/2DJumpActionSample-src.zip?download=` as string | null,
    },
    projectFile: {
      label: "プロジェクト",
      href: `${assetBase}/assets/2DJumpActionSample/2DJumpActionSample-project.zip?download=` as string | null,
    },
  };
  ...
  const items: Item[] = useMemo(
    ...
    export namespace gameSystem
    {
        ...
        export class Execution final
        {
            ...
            <p>
              ● ビルド時間を短縮したい<br />
              - C++20のモジュール機能(.ixxファイル)を活用し、ヘッダーファイルからモジュール化に移行しました。<br />
    ...
    m_ptr = static_cast<char*>(MapViewOfFile(m_mapHandle, FILE_MAP_READ, 0, 0, 0));
            ...
            <p>
              ● ファイル読み込みを最適化したい<br />
              - 大容量のデータファイル読み込みにおいて、メモリマップドファイルを実装しました。<br />
              パフォーマンスが向上し、ゲームの起動時間と動作のスムーズさが改善されました。
            </p>
          </>
        ),
      },
...
co_return;
            ...
            <p>
              ● 非同期処理を簡素化したい<br />
              - コルーチンを導入し、非同期処理における処理を簡素化しました。<br />
...
// フレーム終端でコマンド適用
    ...
    `}
            ...
            <p>
              ● マップデータを効率的に管理したい<br />
              - マップデータの管理にJSON形式を採用し、nlohmann/jsonライブラリを使用ました。<br />
              マップに関するデータの編集が容易になり、開発効率が向上しています。<br />
              - データを格納する際に、チャンク化を行いました。<br />
              チャンク内では一次元配列として管理することで、メモリ使用量とアクセス効率を最適化しました。<br />
              また、チャンク化により、大規模なマップデータの分割管理が可能となり、メモリの断片化を防止しています。<br />
              ● 描画パフォーマンスを向上させたい<br />
              描画範囲を動的に計算し、必要なタイルのみを描画するカリング的処理を導入しました。<br />
            </p>
          </>
        ),
      },
    ...
    ObjCtx::ColMgr().AddEvent(id, col2d::MakeKey(col2d::TILE, ObjectTag::MAP), listener);
            ...
            <p>
              ● 効率的な衝突判定システムを構築したい<br />
              - 独自の2Dコライダーシステムを実装しました。<br />
              コライダーの追加・定義・形状・衝突対象を簡易的に設計することを可能にしました。<br />
              - IDでの管理を採用しました。<br />
              オブジェクトプール方式を用いることで、IDの管理と再利用を効率化しました。<br />
              IDでの参照によりクラス間での依存排除とコライダーのライフサイクル管理を効率化しています。<br />
              ● 衝突判定を適切に行いたい<br />
              - ビジターパターンを採用し、衝突対象ごとに異なる衝突処理を柔軟に実装可能にしました。<br />
              - 衝突判定にはAABB方式などを採用しました。<br />
              スイープや連続衝突検出(CCD)にも対応させることで効率的な衝突検出を実現しました。<br />
              - 衝突イベントをハンドルで管理する仕組みを導入しました。<br />
              衝突イベントの追加や特定の条件下でのイベント発火を柔軟に管理・設定可能にしました。
            </p>
          </>
        ),
      },
    ...
    m_startButtonID = AppCtx::UIMgr().Create<ui::Button>(&startButtonDef);
            ...
            <p>
              ● UI要素のライフサイクル管理を効率化したい<br />
              - コライダーの設計を参考にしました。<br />
              UI管理システムを構築し、UI要素の作成・配置・イベント処理を効率化しました。<br />
              UIコンポーネントのライフサイクル管理が容易になり、コードの可読性と保守性が向上しています。
            </p>
          </>
        ),
      },
...
m_taskInfo[(int)level].tasks.emplace_back(std::async(std::launch::async, [this, level, task]() {
  ...
  if (endInf.finishTasks.load(std::memory_order_relaxed) == endInf.totalTasks.load(std::memory_order_relaxed))
  {
    m_isLoading.store(false, std::memory_order_relaxed);
            ...
            <p>
              ● 処理の非同期化を図りたい<br />
              - asyncとfutureを活用し、非同期処理を実装しました。<br />
              リソースの読み込みやシーン遷移をスムーズに行えるようになりました。<br />
          ...
          <div className="detail-grid">
            ...
            <div className="detail-item">
              ...
              <ul className="bullet-list">
                ...
                <p>
                  <span className="improvement-before">改善前：</span>各クラスが直接他のクラスを参照し、依存関係が複雑化していた。<br />
                  <span className="improvement-after">改善後：</span>ID参照やイベントシステム、コンポーネント指向を導入し、クラス間の依存を最小限に抑制。
                </p>
                <li>C++20のモジュール機能を活用し、ビルド時間の短縮とコードの可読性向上を実現。</li>
                <p>
                  <span className="improvement-before">改善前：</span>C++14を使用しており、ヘッダーファイルの依存関係がビルド時間を増加させていた。<br />
                  <span className="improvement-after">改善後：</span>C++20のモジュール機能を導入し、ヘッダーファイル依存を削減。ビルド時間が大幅に短縮され、コードの可読性と保守性が向上。
                </p>
                <li>テンプレートや関数ポインタを習得。</li>
                <p>
                  <span className="improvement-before">改善前：</span>関数ポインタやテンプレートの使用経験が少なく、コードの柔軟性が低かった。<br />
                  <span className="improvement-after">改善後：</span>関数ポインタやテンプレートを用いた汎用的なコンポーネント設計を実装。コードの柔軟性と再利用性が向上。
                </p>
                <li>コンポーネント指向を採用し、機能の組み合わせの柔軟性とコードの再利用性を向上。</li>
                <p>
                  <span className="improvement-before">改善前：</span>オブジェクト指向設計に基づき、機能がクラスに固定されていた。<br />
                  <span className="improvement-after">改善後：</span>コンポーネント指向を導入し、オブジェクトに機能を動的に追加・組み合わせ可能に。コードの再利用性と拡張性が向上。
                </p>
                <li>メモリマップドファイルを利用して大容量データの読み込みパフォーマンスを最適化。</li>
                <p>
                  <span className="improvement-before">改善前：</span>従来のファイル読み込み方式では、大容量データの読み込みに時間がかかり、パフォーマンスが低下していた。<br />
                  <span className="improvement-after">改善後：</span>メモリマップドファイルを導入し、ファイルI/Oのオーバーヘッドを削減。大容量データの読み込み速度が大幅に向上し、ゲームの起動時間と動作のスムーズさが改善。
                </p>
                <li>非同期処理の実装</li>
                <p>
                  <span className="improvement-before">改善前：</span>同期的な処理が多く、リソースの読み込みやシーン遷移時にゲームが一時停止していた。<br />
                  <span className="improvement-after">改善後：</span>非同期処理を導入。リソースの読み込みやシーン遷移をスムーズに行えるようになり<br />
                  ゲームのパフォーマンスとユーザー体験が向上。
                </p>
                <li>シーン管理の柔軟性と拡張性を向上。</li>
                <p>
                  <span className="improvement-before">改善前：</span>シーン遷移の管理が固定的で、新しいシーン操作の追加が困難であった。<br />
                  <span className="improvement-after">改善後：</span>コマンドパターンを採用し、シーン操作の柔軟性と拡張性を向上。<br />
                  新しいシーン操作の追加が容易になり、コードの保守性が高まった。
                </p>
                <li>マップデータのメモリ使用量とアクセス効率を最適化。</li>
                <p>
                  <span className="improvement-before">改善前：</span>マップデータを二次元配列で管理しており、大規模なマップではメモリ使用量が増加し、アクセス効率が低下していた。<br />
                  <span className="improvement-after">改善後：</span>チャンク化された一次元配列でマップデータを管理。メモリ使用量とアクセス効率が最適化され、大規模なマップデータの分割管理が可能となり、メモリの断片化を防止。
                </p>
                <li>独自の2Dコライダーシステムを実装し、衝突判定の効率化とライフサイクル管理を改善。</li>
                <p>
                  <span className="improvement-before">改善前：</span>衝突判定システムが限定的で、コライダーの管理と衝突処理の拡張が困難であった。<br />
                  <span className="improvement-after">改善後：</span>独自の2Dコライダーシステムを構築。IDでの管理を採用し、コライダーのライフサイクル管理を効率化。<br />
                  ビジターパターンを用いた柔軟な衝突処理の実装が可能となり、システムの拡張性が向上。
                </p>
                <li>UI管理システムを構築</li>
                <p>
                  <span className="improvement-before">改善前：</span>UI要素の管理が分散しており、ライフサイクル管理とイベント処理が煩雑であった。<br />
                  <span className="improvement-after">改善後：</span>UI管理システムを導入し、UI要素の作成・配置・イベント処理を効率化。<br />
                  UIコンポーネントのライフサイクル管理が容易になり、コードの可読性と保守性が向上。
                </p>
              </ul>
            </div>
            ...
            <div className="detail-item">
              ...
              <ul className="bullet-list">
                <li>非同期処理におけるスレッドプールなどが未実装で、パフォーマンスの最適化が不十分でした。</li>
                <li>アニメーションの実装が不十分で、動作が滑らかでない部分がありました。</li>
                <li>衝突判定の精度向上が課題で、特定の状況で誤判定が発生しました。</li>
                </ul>
            </div>
